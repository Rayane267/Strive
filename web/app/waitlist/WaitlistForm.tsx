...
export default function WaitlistForm() {
  ...
  if (state.s === 'done') {
    return (
      <div className="px-2 py-2 text-center">
        ...
        <h2 className="mt-6 text-[1.7rem] font-bold tracking-[-0.02em]">
          {state.already ? 'Tu y étais déjà.' : 'Tu es sur la liste !'}
        </h2>
        <p className="mx-auto mt-3 max-w-sm text-[15px] leading-relaxed text-muted">
          Ta place : <span className="font-bold text-white">#{state.position}</span>. On t&apos;écrit
          dès que Strive ouvre — accès prioritaire pour les premiers inscrits.
        </p>
        <p className="mt-6 text-[13px] text-faint">Pense à vérifier tes spams.</p>
      </div>
    );
  }
  ...
  return (
    <form onSubmit={submit} className="text-center">
      <h2 className="text-[1.75rem] font-bold leading-tight tracking-[-0.025em] sm:text-[2rem]">
        Rejoins la liste d&apos;attente !
      </h2>
      <p className="mx-auto mt-4 max-w-sm text-[15px] leading-relaxed text-muted sm:text-base">
        Sois prévenu dès que Strive ouvre. Scan des courses, €/h réel et trafic temps réel — en
        avant-première.
      </p>
      ...
      <div className="mt-8 space-y-3">
        <input
          ...
          className="wl-input"
        />
        <button type="submit" disabled={state.s === 'loading'} className="wl-btn">
      ...
      {total !== null && total > 0 && (
        <p className="mt-5 text-[13px] text-faint">{total} chauffeurs déjà inscrits</p>
      )}
    </form>
  );
}
